 ...
 */
class ManagerAuthGroupModel extends Model {
    const TYPE_ADMIN                = 1;                   // 管理员用户组类型标识
    const MEMBER                    = 'manager';
    const UCENTER_MEMBER            = 'ucenter_member';
    const AUTH_GROUP_ACCESS         = 'manager_auth_group_access'; // 关系表表名
    const AUTH_EXTEND               = 'manager_auth_extend';       // 动态权限扩展信息表
    const AUTH_GROUP                = 'manager_auth_group';        // 用户组表名
    ...
    public function addToGroup($uid,$gid){
        ...
        if( $del!==false ){
            foreach ($uid_arr as $u){
            	//判断用户id是否合法
            	if(M('Manager')->getFieldByUid($u,'uid') == false){
