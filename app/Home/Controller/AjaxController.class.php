...
class AjaxController extends AdminController{
    ...
    public function ajax_brand(){
        $brand = M('CarBrand')->order('letter asc, sort desc')->select(); 
        $str = '';
        if($brand){
            foreach ($brand as $key=>$value){
                $str .= '<li><a href="javascript:void(0);" onclick="SlectMenu.brand_click(\''. $value['title'] .'\','. $value['id'] .');">'. $value['letter'].' '.$value['title'] .'</a></li>';
            }
        }
        echo $str;
    }
